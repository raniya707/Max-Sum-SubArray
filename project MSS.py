...
//randomized array//
"""     
#Method Array: generates an array with a random size and values  
def Array():  
    
    f = open("MaxSumSubarray.txt", "w") #creat a file named MaxSumSubarray   
    #size: detrmine a random integer length of the array from 10,000 - 50,000
    size= random.randint(0,100)
    # A: creatin an empty array  
    A= []
    #inserting values in the array
    for i in range (0,size):
      #contains random integer valuse from 0 - 100
      A.insert(i, random.randint(0, 100))
    ...
    f = open("/Users/nd3/Desktop/AlgorithmProject/MaxSumSubarray.txt", "r")# we opened the file so we can read the array values
...
def results():
    ...
    arr= Array()#stores the randomaize array, so all algorithms have the same array.
    
    #resultlb1: stores the lable of an informal masseage about kadane(Array()) value
    ...
    #resultlb2: stores the lable of an informal masseage about BruteForceApproach1(Array()) value
    ...
    #resultlb3: stores the lable of an informal masseage about Brute force II(Array()) value
    ...
    #resultlb4: stores the lable of an informal masseage about Aux_array(Array()) value
...
l3 = tk.Label(window,  text='Welcome To Our Program\n please choose one of thoes solutions to test it:', width=33,font=my_font1,
              fg='white',bg='pink4')
